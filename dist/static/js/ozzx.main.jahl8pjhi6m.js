...
if(screenWidth>1000)screenWidth=800;document.getElementById('myCanvas').width=screenWidth;document.getElementById('myCanvas').height=screenWidth*0.6;document.getElementById('myCanvasContainer').style.height=screenWidth*0.6+'px';TagCanvas.Start('myCanvas','tags',{weight:true,fadeIn:1000,weightMode:'both',textColour:'#ff0000',frontSelect:true,outlineMethod:'none',textHeight:20,maxSpeed:0.05,minSpeed:0.005,wheelZoom:false,stretchX:1.8,dragControl:true,weightGradient:{0:'#827e7e',0.33:'#7b6045',0.66:'#b5522a',1:'#632106'}});TagCanvas.SetSpeed('myCanvas',[-0.05,-0.05]);}},"messageBox":{"created":function created(){var html='';// 针对手机和PC分别进行处理
...
tdFlow:{rotate:10,stretch:0,depth:100,modifier:1.4,shadows:true},onSlideClick:function onSlideClick(swiper,e){if(swiper.activeIndex==swiper.clickedSlideIndex-1){var url=swiper.clickedSlide.children[0].src;document.getElementById('showBoxImage').src=url;document.getElementById('closeButton').style.display='block';document.getElementById('showBox').style.top='0';}}});}else{this.data.mySwiper=new Swiper('.swiper-container',{autoplay:3000,loop:true,// 禁用鼠标点击
simulateTouch:false,slidesPerView:3,//其他设置
tdFlow:{rotate:10,stretch:80,depth:30,modifier:1,roundLengths:true},onSlideClick:function onSlideClick(swiper,e){var url=swiper.clickedSlide.getAttribute("src");if(url){window.open(url);}}});}},"last":function last(){this.data.mySwiper.swipePrev();},"next":function next(){this.data.mySwiper.swipeNext();}},"footer":{"created":function created(){}},"showBox":{"created":function created(){},"close":function close(){document.getElementById('closeButton').style.display='none';document.getElementById('showBox').style.top='100%';}}}},"titleBar":{},"copyright":{}},tool:{},entry:"home",state:{},global:{}};// 便捷的获取工具方法
 ...
 * 获取屏幕信息
 * @return {object} 屏幕信息
 */ozzx.tool.getScreenInfo=function(){return{clientWidth:document.body.clientWidth,clientHeight:document.body.clientHeight,ratio:document.body.clientWidth/document.body.clientHeight,// 缩放比例
